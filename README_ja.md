...
## 開発者に選ばれる理由
- プロンプト配線やコンテキスト保存、分岐や採点までを道具箱として提供するため、面倒なグルーコードを書き足す必要がありません。
- 観測はワンアクションで切り替わり、色付きコンソール、SQLite 永続化、OTEL 送信を状況に応じて選べます。
- 失敗時挙動を宣言的に記述でき、実験モードと本番モードの切り替えが設定変更だけで完結します。
- CLI・YAML・Python が同じ操作感で使えるため、試作から運用までスムーズに橋渡しできます。
- プロバイダ固有の癖を内部で吸収し、OpenAI や LMStudio などを同じ手順で切り替えられます。
- 実行時コンテキストがそのままミッションログになり、開発と運用が同じ情報で議論できます。
