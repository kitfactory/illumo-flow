...
## 開発者に選ばれる理由
1. **様々な言語モデルを簡単に**: ランタイムがプロバイダ差分を吸収するので、OpenAI / Anthropic / LM Studio / Ollama などを切り替えても `/v1` などの細かい違いを気にせず同じコードで実験から本番まで運べます。
2. **エージェントのフロー制御を簡単に**: フロー定義を DSL でさらりと書き、オーケストレーターから明示制御できるため、エージェントごとの処理が見通し良く管理できます。
3. **トレース（記録・デバッグ）が簡単に**: 色付きコンソールの会話ログから SQLite 永続化（将来的には OTEL 連携）までワンフリップ。print 依存のデバッグから卒業できます。
4. **制御管理の切り替えが簡単に**: 開発中は緩め、本番では厳格な Fail-Fast／リトライ設定へ。設定値を入れ替えるだけでフロー制御ポリシーを素早く切り替えられます。
5. **CLI・YAML・Python が地続き**: 同じ設計思想で利用できるため、朝の試作を夕方の運用にそのまま引き継げます。
6. **コンテキストがミッションログに**: 実行履歴がそのまま閲覧できるため、「何が起きたか」をすぐ追跡し、次のアクションを判断できます。
